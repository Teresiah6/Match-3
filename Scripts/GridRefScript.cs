...
public class GridRefScript : MonoBehaviour
{
    ...
    {
        EMPTY, NORMAL, COUNT, BUBBLE, ROW_CLEAR, COLUMN_CLEAR, RAINBOW,
    };
    [System.Serializable]
    ...
    public void SwapPieces(GamePiece piece1, GamePiece piece2)
    {
        ...
        if (piece1.IsMovable() && piece2.IsMovable())
        {
               ...
               || piece1.Type ==PieceType.RAINBOW || piece2.Type == PieceType.RAINBOW)
            {
                ...
                //check if rainbow piece and clearable
                ...
                //check if pieces clearing the match are row or colum clearing pieces
                ...
                //more matches could be made
                StartCoroutine(Fill());
                level.OnMove();
            }
            else
            {
                //swap them back if not a match
    ...
    public List<GamePiece> GetMatch(GamePiece piece, int newX, int newY)
    {
        ...
        if (piece.IsColored())
        {
            ...
            {
                //loop through the adjacent peices
            ...
            //if has enough pieces to make a match
            ...
            //L and T matches transverse vertically
            if (horizontalPieces.Count >= 3)
            {
                for (int i = 0; i < horizontalPieces.Count; i++)
                {
                    ...
                    //check if we have enough vertical pieces to form a match
            ...
            //clear horizontal and vertical arrays to get ready to transverse again
            ...
            //L and T matches transverse horizontally
            if (verticalPieces.Count >= 3)
            {
                for (int i = 0; i < verticalPieces.Count; i++)
                {
                    ...
                    //check if we have enough horizontal pieces to form a match
    ...
    //check whole board for clearable piece
        ...
        for (int y = 0; y < yDim; y++)
        {
            for (int x = 0; x < xDim; x++)
            {
                if (pieces[x, y].IsClearable())
                {
                    ...
                    //if match was found, loop through matches and clear
                    if (match != null)
                    {
                        //using special pieces
                        ...
                        for (int i = 0; i < match.Count; i++)
                        {
                            if (ClearPiece(match[i].X, match[i].Y))
                            {
                                ...
                                //set the position of the special piece
                        ...
                        //check whether you should spawn special piece
                        if(specialPieceType != PieceType.COUNT)
                        {
                            Destroy(pieces[specialPieceX, specialPieceY]);
                            GamePiece newPiece = SpawnNewPiece(specialPieceX, specialPieceY, specialPieceType);
                            //color
                            if(specialPieceType==PieceType.ROW_CLEAR || specialPieceType ==PieceType.COLUMN_CLEAR && newPiece.IsColored()
                                && match[0].IsColored())
                            {
    ...
    //function to clear piece
    public bool ClearPiece(int x, int y)
    {
        //check if piece is clearable and hasn't been marked for clearing
        if (pieces[x, y].IsClearable() && !pieces[x, y].ClearableComponent.IsBeingCleared)
        {
            //call clear on piece and spawn new
    ...
    {
        //loop all the pieces in the grid
        for(int x = 0; x<xDim; x++)
        {
            for(int y =0; y<yDim; y++)
            {
    ...
    public List<GamePiece> GetPiecesOfType(PieceType type)
    {
        ...
        for(int x =0; x<xDim; x++)
        {
            for(int y =0; y<yDim; y++)
            {
                ...
                {
                    //if pieces are the same incrment 
